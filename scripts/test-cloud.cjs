...
const jwt=()=>[Buffer.from(JSON.stringify({alg:'HS256',typ:'JWT'})).toString('base64url'),Buffer.from(JSON.stringify({sub:USER,role:'authenticated',exp:Math.floor(Date.now()/1000)+3600})).toString('base64url'),'test-signature'].join('.');
...
const session=()=>({access_token:jwt(),refresh_token:'test-only-refresh',token_type:'bearer',expires_in:3600,user});
...
if(require.main===module)(async()=>{
 ...
 async function login(page){await page.locator('#loginEmail').fill('family@example.invalid');await page.locator('#loginPassword').fill('test-password-only');await page.locator('#loginButton').click();await page.locator('#boardApp').waitFor({state:'visible'});await waitFor(async()=>!(await page.locator('#syncStatus').innerText()).includes('保存')&&!(await page.locator('#syncStatus').innerText()).includes('저장 중'),'initial sync');}
 ...
 try{
  ...
  assert(await a.locator('#loginPanel').isVisible());assert(!(await a.locator('#boardApp').isVisible()));
  await a.locator('#loginEmail').fill('family@example.invalid');await a.locator('#loginPassword').fill('incorrect');await a.locator('#loginButton').click();await a.getByText('이메일과 비밀번호를 다시 확인해 주세요.').waitFor();
  ...
  await waitFor(async()=>!(await b.evaluate(()=>Object.keys(localStorage).some(k=>k.includes(':op:')))),'all offline changes acknowledged');
  ...
  await a.locator('#logoutButton').click();await a.locator('#loginPanel').waitFor({state:'visible'});assert(!(await a.locator('#boardApp').isVisible()));await a.reload();assert(await a.locator('#loginPanel').isVisible());
  assert.equal(await a.evaluate(()=>document.documentElement.scrollWidth<=innerWidth),true,'375px login layout');await a.screenshot({path:path.join(os.tmpdir(),'heeyoon-cloud-login.png'),fullPage:true});
  ...
  const {page:unconfigured}=await open({},false);assert(await unconfigured.locator('#loginButton').isDisabled());assert(!(await unconfigured.locator('#boardApp').isVisible()));
  assert.deepEqual(errors,[],'no uncaught page errors, even with invalid local data');
  console.log('PASS: isolated Supabase simulation A-J, 375px, exact PK/revision writes, no legacy uploads, local failure/reload retry, conflict protection, account logout, original backup, subject keys unchanged.');
  console.log('Screenshots: '+path.join(os.tmpdir(),'heeyoon-cloud-login.png')+' ; '+path.join(os.tmpdir(),'heeyoon-cloud-main.png'));
